...
import { ArrowRight, ChevronDown, LogOut, Minus, Plus, Search, ShoppingBag, Trash2, UserRound, X } from "lucide-react";
...
function AccountAccess() {
  ...
  useEffect(() => {
    ...
    const loadAccount = async () => {
      ...
      if (!active || !user) {
        if (active) setAccount({ signedIn: false, displayName: "", photoUrl: null });
        return;
      }
      const { data: profile } = await client.from("profiles").select("display_name, profile_photo_path").eq("id", user.id).maybeSingle();
      const photoPath = profile?.profile_photo_path;
      const photoUrl = photoPath ? client.storage.from("customer-profile-photos").getPublicUrl(photoPath).data.publicUrl : null;
      if (active) setAccount({ signedIn: true, displayName: profile?.display_name ?? "", photoUrl });
    };
    void loadAccount();
  ...
  return <div className="relative"><button type="button" onClick={() => setOpen((value) => !value)} className="icon-button overflow-hidden" aria-label="Abrir minha conta" aria-expanded={open}>{account.photoUrl ? <img src={account.photoUrl} alt="" className="h-5 w-5 rounded-full object-cover" /> : initials ? <span className="text-[9px] font-black text-[#7affb9]">{initials}</span> : <UserRound size={19} />}</button>{open && <div className="absolute right-0 top-11 z-50 w-52 rounded-2xl border border-white/10 bg-[#151a1f] p-2 shadow-2xl"><p className="px-3 pb-2 pt-1 text-xs text-white/45">{account.displayName || "Minha conta"}</p><Link href="/perfil" onClick={() => setOpen(false)} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold text-white/80 transition hover:bg-white/[0.07] hover:text-[#7affb9]"><UserRound size={16} /> Minha conta</Link><button type="button" onClick={() => void signOut()} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-sm text-white/65 transition hover:bg-white/[0.07] hover:text-red-300"><LogOut size={16} /> Sair</button></div>}</div>;
}
...
function Footer({ settings, logoUrl }: { settings: StorefrontSettings; logoUrl: string }) {
  return <footer className="border-t border-white/10 bg-[#080a0c]"><div className="container grid gap-10 py-12 md:grid-cols-[1.1fr_.8fr_.8fr]"><div><div className="flex items-center gap-3"><img src={logoUrl} alt="Logo IM" className="h-11 w-11 rounded-full object-cover" /><div><p className="font-semibold tracking-[0.18em]">IAGO MODAS</p><p className="mt-1 text-xs text-white/45">Moda masculina com presença.</p></div></div><p className="mt-5 max-w-sm text-sm leading-6 text-white/55">{settings.footer_description}</p></div><div><p className="eyebrow">NAVEGUE</p><div className="mt-4 grid grid-cols-2 gap-x-5 gap-y-3">{categories.slice(0, 6).map((category) => <Link key={category} href={`/categoria/${categorySlug(category)}`} className="text-sm text-white/60 transition hover:text-[#7affb9]">{category}</Link>)}</div></div><div><p className="eyebrow">ATENDIMENTO</p>{settings.instagram_enabled && <button type="button" onClick={() => openInstagramApp(settings.instagram_handle)} className="mt-4 inline-flex items-center gap-2 text-sm text-white/70 transition hover:text-[#7affb9]"><FaInstagram aria-hidden="true" size={17} /> Instagram @{settings.instagram_handle} <ChevronDown className="-rotate-90" size={15} /></button>}{settings.whatsapp_enabled && settings.whatsapp_number && <button type="button" onClick={() => openWhatsAppChat(settings.whatsapp_number)} className="mt-3 inline-flex items-center gap-2 text-sm text-white/70 transition hover:text-[#7affb9]"><FaWhatsapp aria-hidden="true" size={17} /> Falar no WhatsApp <ChevronDown className="-rotate-90" size={15} /></button>}{!settings.instagram_enabled && !(settings.whatsapp_enabled && settings.whatsapp_number) && <p className="mt-4 text-sm text-white/45">Atendimento temporariamente indisponível.</p>}<p className="mt-4 whitespace-pre-line text-sm leading-6 text-white/45">{settings.footer_location}</p>{settings.footer_hours.trim() && <p className="mt-2 text-xs text-white/40">{settings.footer_hours}</p>}</div></div><div className="border-t border-white/10 py-5 text-center text-xs text-white/35">© {new Date().getFullYear()} IAGO MODAS. Todos os direitos reservados.</div></footer>;
}
