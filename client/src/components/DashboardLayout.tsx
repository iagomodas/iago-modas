...
export type DashboardMenuItem = { icon: LucideIcon; label: string; target: string };
...
export default function DashboardLayout({
  ...
  return (
    ...
    >
      <DashboardLayoutContent brandLabel={brandLabel} menuItems={menuItems} setSidebarWidth={setSidebarWidth}>
        {children}
...
function DashboardLayoutContent({
  ...
  const [activeMenuTarget, setActiveMenuTarget] = useState(menuItems[0]?.target ?? "");
  const activeMenuItem = menuItems.find(item => item.target === activeMenuTarget);
              ...
              {menuItems.map(item => {
                return (
                  <SidebarMenuItem key={item.target}>
                    <SidebarMenuButton
                      isActive={item.target === activeMenuTarget}
                      onClick={() => {
                    ...
                    >
                      <item.icon
                        className={`h-4 w-4 ${item.target === activeMenuTarget ? "text-primary" : ""}`}
                      />
