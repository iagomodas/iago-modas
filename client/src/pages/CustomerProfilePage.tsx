...
const PROFILE_PHOTO_HINT = "Use JPG, PNG ou WEBP. A foto é otimizada antes do envio para ocupar pouco espaço sem perder a aparência de perfil.";
...
export default function CustomerProfilePage() {
    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        if (active) {
          ...
          setPhotoPath(null);
          setPhotoUrl(null);
          setNotice("Não foi possível carregar seu cadastro agora. Verifique a conexão e tente novamente.");
        }
      } finally {
        if (active) setLoading(false);
      }
  ...
  async function changePhoto(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("customer-profile-photos").upload(nextPath, optimized.file, { upsert: true, contentType: "image/webp", cacheControl: "3600" });
      ...
      const storedPhotoUrl = await createPrivateProfilePhotoUrl(supabase, nextPath);
