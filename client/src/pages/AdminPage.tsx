...
import { Boxes, Brush, ClipboardList, FileText, LayoutDashboard, Loader2, Megaphone, PackagePlus, Pencil, ReceiptText, Save, ShieldAlert, Trash2 } from "lucide-react";
import { type FormEvent, type ReactNode, useEffect, useMemo, useState } from "react";
import { Link } from "wouter";

type ProductForm = { name: string; slug: string; category: Category; brand: string; collection: string; description: string; price: string; compareAtPrice: string; sizes: string; imageUrl: string; imageUrls: string; badge: string; accentColor: string; stock: string; isActive: boolean };
type StorefrontField = keyof StorefrontSettings;

const emptyForm: ProductForm = { name: "", slug: "", category: "Camisetas", brand: "IAGO MODAS", collection: "", description: "", price: "", compareAtPrice: "", sizes: "P, M, G, GG", imageUrl: "", imageUrls: "", badge: "", accentColor: "#82ffc5", stock: "0", isActive: true };
...
const operationalStatuses = ["awaiting_freight", "freight_informed", "awaiting_pix", "paid", "ready_to_post", "shipped", "cancelled"] as const;
type OperationalStatus = typeof operationalStatuses[number];
const operationalLabels: Record<OperationalStatus, string> = { awaiting_freight: "Frete a combinar pelo Instagram", freight_informed: "Frete informado", awaiting_pix: "Aguardando Pix", paid: "Pix confirmado", ready_to_post: "Pronto para postar", shipped: "Postado", cancelled: "Cancelado" };

function toForm(product: SupabaseProduct): ProductForm {
  return { name: product.name, slug: product.slug, category: product.category as Category, brand: product.brand ?? "IAGO MODAS", collection: product.collection ?? "", description: product.description, price: (product.price_cents / 100).toFixed(2).replace(".", ","), compareAtPrice: product.compare_at_price_cents ? (product.compare_at_price_cents / 100).toFixed(2).replace(".", ",") : "", sizes: Array.isArray(product.sizes) ? product.sizes.join(", ") : String(product.sizes ?? ""), imageUrl: product.image_url, imageUrls: Array.isArray(product.image_urls) ? product.image_urls.join("\n") : "", badge: product.badge ?? "", accentColor: product.accent_color, stock: String(product.stock), isActive: product.is_active };
}
...
export default function AdminPage() {
  ...
  return <DashboardLayout brandLabel="IAGO MODAS" menuItems={adminMenu}><AdminConsole /></DashboardLayout>;
}
...
function AccessDenied() {
  return <main className="grid min-h-screen place-items-center bg-[#080909] px-5 text-center text-white"><section className="max-w-md rounded-3xl border border-red-300/20 bg-red-300/[.04] p-8"><ShieldAlert className="mx-auto text-red-300" size={38} /><h1 className="mt-5 font-display text-3xl">Acesso restrito</h1><p className="mt-3 text-sm leading-6 text-white/60">Esta conta não possui a permissão de administrador da IAGO MODAS.</p><Link href="/"><Button variant="outline" className="mt-7 border-white/20 text-white hover:bg-white/10">VOLTAR PARA A LOJA</Button></Link></section></main>;
}
...
function AdminConsole() {
  ...
  const updateOperationalStatus = async (id: number, status: OperationalStatus) => {
    if (!supabase) return;
    const { error } = await supabase.from("orders").update({ order_status: status }).eq("id", id);
    setFeedback(error ? error.message : "Etapa operacional do pedido atualizada."); await operations.refresh();
  };

  return <div className="mx-auto max-w-7xl space-y-8 pb-10 text-white">
    <header className="flex flex-wrap items-end justify-between gap-4 border-b border-white/10 pb-6"><div><p className="text-xs font-bold tracking-[.2em] text-[#82ffc5]">IAGO MODAS</p><h1 className="mt-2 font-display text-4xl">Controle da loja</h1><p className="mt-2 max-w-2xl text-sm text-white/55">Edite produtos, preços, estoque, pedidos, vitrine e etiquetas de postagem manual.</p></div><Link href="/"><Button variant="outline" className="border-white/20 text-white hover:bg-white/10">VER LOJA</Button></Link></header>
    <AdminSalesOverview orders={operations.orders} loading={operations.loading} />
    ...
    <section id="orders" className="scroll-mt-24 overflow-hidden rounded-3xl border border-white/10 bg-white/[.035]"><div className="border-b border-white/10 px-6 py-5"><SectionHeading eyebrow="PEDIDOS" title="Acompanhamento" icon={<ClipboardList size={18} />} description="Consulte o endereço, combine o frete manualmente pelo Instagram, confirme o Pix, registre a postagem e carregue a etiqueta pronta para impressão." /></div>{operations.error ? <ErrorText message={operations.error} /> : <div className="divide-y divide-white/10">{operations.orders.map((order) => <div key={order.id} className="flex flex-wrap items-center gap-x-5 gap-y-3 px-6 py-4 text-sm"><div className="min-w-36"><p className="font-medium">{order.order_number}</p><p className="mt-1 text-xs text-white/45">{new Date(order.created_at).toLocaleDateString("pt-BR")}</p></div><div className="min-w-40 flex-1"><p className="text-white/80">{order.customer_name}</p><p className="mt-1 text-xs text-white/45">{order.delivery_mode === "correios" ? `${order.address ?? ""}, ${order.delivery_number ?? ""} · ${order.delivery_city ?? ""}/${order.delivery_state ?? ""}` : `${order.delivery_mode === "city_delivery" ? "Entrega" : "Retirada"} em ${storefront.settings.local_city} — combinar pelo Instagram`}</p></div><Button type="button" variant="outline" className="border-white/20 text-xs text-white hover:bg-white/10" onClick={() => { window.sessionStorage.setItem("oversized-label-order", JSON.stringify(order)); document.getElementById("labels")?.scrollIntoView({ behavior: "smooth" }); }}>ETIQUETA</Button><label className="grid gap-1 text-[10px] font-bold tracking-wide text-white/45">PAGAMENTO<select value={order.payment_status} onChange={(event) => void updateOrder(order.id, event.target.value as SupabaseOrder["payment_status"])} className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-xs font-normal text-white outline-none"><option value="pending">{orderLabels.pending}</option><option value="approved">{orderLabels.approved}</option><option value="rejected">{orderLabels.rejected}</option><option value="cancelled">{orderLabels.cancelled}</option></select></label><label className="grid gap-1 text-[10px] font-bold tracking-wide text-white/45">OPERAÇÃO<select value={(operationalStatuses.includes(order.order_status as OperationalStatus) ? order.order_status : "awaiting_freight") as OperationalStatus} onChange={(event) => void updateOperationalStatus(order.id, event.target.value as OperationalStatus)} className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-xs font-normal text-white outline-none">{operationalStatuses.map((status) => <option key={status} value={status}>{operationalLabels[status]}</option>)}</select></label><p>{toMoney(order.total_cents / 100)}</p></div>)}{!operations.loading && operations.orders.length === 0 && <Empty message="Ainda não há pedido registrado. Os clientes iniciarão o pedido pela conversa no Instagram." />}</div>}</section>
    <ShippingLabelGenerator />
    <section id="product-organization" className="scroll-mt-24 rounded-3xl border border-white/10 bg-white/[.035] p-6 md:p-7"><SectionHeading eyebrow="ORGANIZAÇÃO DO CATÁLOGO" title="Marca e coleção" icon={<Boxes size={18} />} description="Use estes dados para qualquer item da loja: camisas, calças, bermudas, calçados, acessórios, perfumes ou kits. Depois, salve o produto no formulário do catálogo acima." /><div className="mt-6 grid gap-4 md:grid-cols-2"><Field label="Marca"><input value={form.brand} onChange={(event) => set("brand", event.target.value)} className="admin-input" placeholder="Ex.: IAGO MODAS" /></Field><Field label="Coleção"><input value={form.collection} onChange={(event) => set("collection", event.target.value)} className="admin-input" placeholder="Ex.: Drop Inverno 2026" /></Field></div><p className="mt-4 text-xs leading-5 text-white/50">A marca começa preenchida como IAGO MODAS. A coleção é opcional e pode ser usada para organizar drops, lançamentos ou linhas especiais.</p><Button type="button" variant="outline" onClick={() => document.getElementById("products")?.scrollIntoView({ behavior: "smooth" })} className="mt-5 border-white/20 text-white hover:bg-white/10">VOLTAR AO CADASTRO PARA SALVAR</Button></section>
  </div>;
}
...
function StorefrontEditor({ draft, onFieldChange, onImageChange, onSave, saving, error }: { draft: StorefrontSettings; onFieldChange: (key: StorefrontField, value: string | boolean) => void; onImageChange: (value: string | null) => void; onSave: (event: FormEvent) => void; saving: boolean; error: string | null }) {
  const groups: { title: string; fields: { key: StorefrontField; label: string; multiline?: boolean }[] }[] = [
    ...
    { title: "ATENDIMENTO, ENTREGA E PIX", fields: [{ key: "local_city", label: "Cidade para entrega ou retirada local" }, { key: "local_state", label: "UF da cidade local" }, { key: "local_pickup_label", label: "Texto da opção de retirada local" }, { key: "local_delivery_label", label: "Texto da opção de entrega na cidade" }, { key: "outside_delivery_label", label: "Texto da opção para outra cidade" }, { key: "outside_delivery_notice", label: "Aviso de frete para outra cidade", multiline: true }, { key: "pix_key", label: "Chave Pix ou instrução de pagamento" }] },
  ];
  return <form onSubmit={onSave} className="mt-7 grid gap-5">{groups.map((group) => <fieldset key={group.title} className="grid gap-4 rounded-2xl border border-white/10 p-5"><legend className="px-2 text-xs font-bold tracking-[.16em] text-[#82ffc5]">{group.title}</legend>{group.title === "AVISO E DESTAQUE PRINCIPAL" && <><Field label="Logo da loja (URL opcional)"><input type="url" value={draft.logo_url ?? ""} onChange={(event) => onFieldChange("logo_url", event.target.value.trim())} className="admin-input" placeholder="https://.../logo.png" /><p className="-mt-2 text-xs leading-5 text-white/45">Cole a URL da imagem da logo. Se ficar vazia, a logo padrão continuará sendo usada.</p></Field><Field label="Imagem principal (URL opcional)"><input type="url" value={draft.hero_image_url ?? ""} onChange={(event) => onImageChange(event.target.value.trim() || null)} className="admin-input" placeholder="https://..." /></Field><div className="grid gap-4 md:grid-cols-2"><Field label="Cor principal"><input type="color" value={draft.primary_color} onChange={(event) => onFieldChange("primary_color", event.target.value)} className="h-11 w-full cursor-pointer rounded-lg border border-white/10 bg-black/30 p-1" /></Field><Field label="Cor de fundo"><input type="color" value={draft.background_color} onChange={(event) => onFieldChange("background_color", event.target.value)} className="h-11 w-full cursor-pointer rounded-lg border border-white/10 bg-black/30 p-1" /></Field></div><div className="grid gap-3 rounded-xl border border-white/10 bg-black/20 p-4 sm:grid-cols-2"><p className="text-xs font-bold tracking-[.16em] text-[#82ffc5] sm:col-span-2">SEÇÕES VISÍVEIS NA HOME</p>{([['hero_visible','Banner principal'],['promotion_visible','Promoção da semana'],['highlights_visible','Destaques'],['benefits_visible','Benefícios'],['newsletter_visible','Novidades']] as const).map(([key, label]) => <label key={key} className="flex items-center gap-3 text-sm text-white/70"><input type="checkbox" checked={draft[key]} onChange={(event) => onFieldChange(key, event.target.checked)} />{label}</label>)}</div></>}<div className="grid gap-4 md:grid-cols-2">{group.fields.map((field) => <Field key={field.key} label={field.label}>{field.multiline ? <textarea required value={String(draft[field.key] ?? "")} onChange={(event) => onFieldChange(field.key, event.target.value)} className="admin-input min-h-20 py-2" /> : <input required value={String(draft[field.key] ?? "")} onChange={(event) => onFieldChange(field.key, event.target.value)} className="admin-input" />}</Field>)}</div></fieldset>)}{error && <ErrorText message={error} />}<div><Button disabled={saving} className="bg-[#82ffc5] text-black hover:bg-white">{saving ? <Loader2 className="mr-2 animate-spin" size={16} /> : <Save className="mr-2" size={16} />}SALVAR VITRINE</Button></div></form>;
}

function LocalDeliveryControls({ draft, onFieldChange }: { draft: StorefrontSettings; onFieldChange: (key: StorefrontField, value: string | boolean) => void }) {
  return <div className="mt-6 grid gap-4 rounded-2xl border border-white/10 bg-black/20 p-5 sm:grid-cols-2">
    <p className="text-xs font-bold tracking-[.16em] text-[#82ffc5] sm:col-span-2">ATENDIMENTO E OPÇÕES LOCAIS</p>
    <Field label="Usuário do Instagram"><input value={draft.instagram_handle} onChange={(event) => onFieldChange("instagram_handle", event.target.value.replace(/^@/, ""))} className="admin-input" placeholder="iagomodas9" /></Field>
    <Field label="Número do WhatsApp (DDD + número)"><input value={draft.whatsapp_number} onChange={(event) => onFieldChange("whatsapp_number", event.target.value.replace(/\D/g, ""))} inputMode="tel" className="admin-input" placeholder="Ex.: 82999999999" /></Field>
    <label className="flex items-center gap-3 text-sm text-white/70"><input type="checkbox" checked={draft.instagram_enabled} onChange={(event) => onFieldChange("instagram_enabled", event.target.checked)} />Exibir Instagram</label>
    <label className="flex items-center gap-3 text-sm text-white/70"><input type="checkbox" checked={draft.whatsapp_enabled} onChange={(event) => onFieldChange("whatsapp_enabled", event.target.checked)} disabled={!draft.whatsapp_number} />Exibir WhatsApp</label>
    <label className="flex items-center gap-3 text-sm text-white/70"><input type="checkbox" checked={draft.local_pickup_enabled} onChange={(event) => onFieldChange("local_pickup_enabled", event.target.checked)} />Permitir retirada local</label>
    <label className="flex items-center gap-3 text-sm text-white/70"><input type="checkbox" checked={draft.local_delivery_enabled} onChange={(event) => onFieldChange("local_delivery_enabled", event.target.checked)} />Permitir entrega na cidade</label>
    <p className="text-xs leading-5 text-white/45 sm:col-span-2">Você pode manter Instagram e WhatsApp ativos ao mesmo tempo. Sem número, o WhatsApp fica oculto. Use “Salvar vitrine” abaixo para aplicar as alterações.</p>
  </div>;
}
