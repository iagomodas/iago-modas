...
export default function SearchPage() {
  ...
  const results = hasQuery ? products.filter((product) => `${product.name} ${product.category} ${product.brand ?? ""} ${product.collection ?? ""} ${product.description}`.toLocaleLowerCase("pt-BR").includes(normalizedQuery)) : [];
